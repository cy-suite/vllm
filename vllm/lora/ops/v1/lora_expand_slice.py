...
def _lora_expand_slice_kernel(
    ...
    c_mask = (offset_cm[:, None] < cta_m_size) & (offset_cn[None, :] < (N + slice_offset))
    if ADD_INPUTS:
...
def _lora_expand_slice(
    ...
    _lora_expand_slice_kernel[grid](
        inputs,
        lora_b_weights,
        output_tensor,
        M,
...
try:
    direct_register_custom_op(
        op_name="lora_expand_slice",
        op_func=_lora_expand_slice,
        mutates_args=["output_tensor"],
        fake_impl=lora_expand_slice_fake,
    )
    lora_expand_slice = torch.ops.vllm.lora_expand_slice

except AttributeError:
    lora_expand_slice = _lora_expand_slice